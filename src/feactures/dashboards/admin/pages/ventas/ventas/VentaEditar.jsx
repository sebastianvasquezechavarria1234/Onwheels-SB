                            ...
                            >
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50">
                                    {filteredProducts.map(product => {
                                        ...
                                        return (
                                            <div key={product.id_producto} className={`bg-white border rounded-xl overflow-hidden transition ${isSelected ? 'border-blue-500 ring-1 ring-blue-500 shadow-md' : 'border-gray-200 shadow-sm'}`}>
                                                <div className="p-3 flex items-start gap-3 border-b border-gray-100 bg-gray-50">
                                                    <input
                                                        type="checkbox"
                                                        checked={isSelected}
                                                        onChange={(e) => handleProductSelect(product.id_producto, e.target.checked)}
                                                        className="mt-1 w-4 h-4 text-blue-600 rounded"
                                                    />
                                                    <div>
                                                        <h4 className="font-semibold text-gray-800">{product.nombre_producto}</h4>
                                                        <p className="text-sm text-green-600 font-medium">${Number(product.precio_venta).toLocaleString()}</p>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div className="p-3 space-y-2 bg-blue-50/30">
                                                        {selectedProducts[product.id_producto].variantes.map((v, idx) => (
                                                            <div key={idx} className="flex flex-wrap items-center gap-2 p-2 bg-white rounded-lg border border-gray-200">
                                                                ...
                                                                <button
                                                                    onClick={() => removeVariantRow(product.id_producto, idx)}
                                                        ...
                                                        <button
                                                            onClick={() => addVariantToProduct(product.id_producto)}
