...
function Ventas() {
  ...
  const getStatusStyle = (estado) => {
    switch (estado) {
      ...
      case "Pendiente": return configUi.pill + " bg-amber-50 text-amber-700 border-amber-100";
                ...
                <tr>
                   <td colSpan="7" className={configUi.emptyState}>
                     <div className="flex flex-col items-center gap-3 opacity-20">
                        <ShoppingBag size={48} />
                        <p className="text-xs font-black uppercase tracking-widest leading-none">Sin transacciones registradas</p>
                     </div>
                   </td>
                </tr>
              ) : (
                  ...
                  <tr key={v.id_venta} className={configUi.row}>
                    ...
                    <td className={configUi.td}>
                      <div className="flex flex-col">
                         <span className="font-extrabold text-[#16315f] font-mono leading-none">#{String(v.id_venta).padStart(6, '0')}</span>
                         <span className="text-[10px] text-[#6b84aa] flex items-center gap-1 mt-1 font-bold">
                            <Calendar size={10} className="opacity-40" /> {new Date(v.fecha_venta).toLocaleDateString()}
                         </span>
                      </div>
                    </td>
                    <td className={configUi.td}>
                      <div className="flex flex-col">
                         <span className="font-bold text-[#16315f] truncate max-w-[200px] leading-none">{getClienteNombre(v.id_cliente)}</span>
                         <span className="text-[10px] text-[#6b84aa] font-bold uppercase mt-1 tracking-tighter">Vía {v.metodo_pago}</span>
                      </div>
                    </td>
                    <td className={configUi.td + " text-center"}>
                      <span className={configUi.subtlePill}>
                         {v.items?.length || 0} UNI
                      </span>
                    </td>
                    ...
                    <td className={configUi.td + " text-center"}>
                      <span className={cn(getStatusStyle(v.estado), "text-[9px] min-w-[80px] justify-center")}>
                    ...
                    <td className={configUi.td + " text-right"}>
                      <div className="flex items-center justify-end gap-1">
                        <button 
                          onClick={() => navigate(`${basePath}/ventas/detalle/${v.id_venta}`)} 
                  ...
                  {modal === 'cancelar' ? (
                     <div className="space-y-6">
                        ...
                        <div className="text-center">
                           <p className="text-sm font-black text-[#16315f] uppercase tracking-tight">¿Confirmar Anulación?</p>
                           <p className="text-[11px] text-[#6b84aa] mt-1">Esta acción revertirá el stock y dejará la factura sin efecto contable.</p>
                        </div>
