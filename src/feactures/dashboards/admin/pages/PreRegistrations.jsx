...
export const PreRegistrations = () => {
    ...
    const saveAdd = () => {
        ...
        const newRole = { id: newId, nombre: addForm.nombre || `Rol ${newId}`, descripcion: addForm.descripcion || "", estado: typeof addForm.estado === "boolean" ? addForm.estado : true };
    ...
    return (
        <Layout>
            <section className="dashboard__pages relative w-full overflow-y-scroll sidebar h-screen">
                <h2 className="dashboard__title font-primary p-[30px] font-secundaria">Clases / Preinscripciones</h2>
                ...
                <div className="p-[30px]">
                    ...
                    {roles.map((role) => (
                        <article key={role.id} className="py-[18px] border-b border-black/20 flex items-center">
                            ...
                            <p className="w-[15%]">
                                <span onClick={() => toggleEstado(role.id)} className={`px-[15px] py-[7px] rounded-full inline-flex items-center gap-[10px] cursor-pointer ${role.estado ? "bg-green-100 text-green-700 border border-green-200" : "bg-red-100 text-red-700 border border-red-200"}`}>
