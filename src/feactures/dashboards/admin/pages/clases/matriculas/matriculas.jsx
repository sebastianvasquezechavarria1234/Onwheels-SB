...
const MatriculasAdmin = () => {
  ...
  // Para nuevo estudiante (usuario existente)
  ...
  // Para editar — se precargan al abrir el modal ✅
    ...
    try {
      ...
      const estudiantesConMatriculas = Array.from(estudiantesMap.values())
        .map((est) => {
          ...
          return { ...est, matriculaPrincipal: est.matriculas[0] };
        })
        .sort(
          (a, b) =>
            new Date(b.matriculaPrincipal.fecha_matricula) -
            new Date(a.matriculaPrincipal.fecha_matricula)
        );
  ...
  // ─── Fetch datos para selects ────────────────────────────────────────────────

  const fetchDatosParaClase = useCallback(async () => {
    try {
      const config = getAuthHeader();
      const [clasesData, planesData, estudiantesData, usuariosElegiblesData] =
        await Promise.all([
          axios.get("http://localhost:3000/api/clases", config).then((r) => r.data),
          axios.get("http://localhost:3000/api/planes", config).then((r) => r.data),
          axios.get("http://localhost:3000/api/estudiantes", config).then((r) => r.data),
          axios
            .get("http://localhost:3000/api/usuarios/elegibles-para-estudiante", config)
            .then((r) => r.data),
        ]);
      ...
      showNotification("Error al cargar datos de formulario", "error");
    }
  }, []);
  ...
  // ─── useEffect corregido con ambas dependencias ✅ ───────────────────────────
  useEffect(() => {
    fetchMatriculas();
    fetchDatosParaClase();
  }, [fetchMatriculas, fetchDatosParaClase]);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const msg = err.response?.data?.mensaje || "Error al actualizar matrícula";
      showNotification(msg, "error");
    }
  };

  // ─── Crear para estudiante existente ────────────────────────────────────────
  ...
  // ─── Crear para usuario existente (nuevo estudiante) ────────────────────────
  const handleCrearParaUsuarioExistente = async () => {
    ...
    try {
      const config = getAuthHeader();
      await axios.post(
        "http://localhost:3000/api/matriculas/manual",
        {
          ...
          acudiente_telefono: acudienteTelefono || null,
          acudiente_email: acudienteEmail || null,
          id_clase: parseInt(claseSeleccionada),
          id_plan: parseInt(planSeleccionado),
        },
        config
  ...
  const estadoBadge = (estado) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs ${clases[estado] ?? "bg-gray-100 text-gray-800"}`}>
        {estado}
      </span>
    );
  };
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div className="relative w-full md:w-96">
              ...
              <input
                ...
                placeholder="Buscar (Estudiante, Documento, Plan, Nivel)"
                ...
                <tbody>
                  {loading ? (
                    ...
                    currentItems.map((est) => {
                      const mp = est.matriculaPrincipal;
                      return (
                        <React.Fragment key={est.id_estudiante}>
                          <tr className="border-b hover:bg-gray-50 transition bg-blue-50">
                            ...
                            <td className="px-6 py-4">{mp.nombre_nivel} - {mp.dia_semana} {mp.hora_inicio}</td>
                            <td className="px-6 py-4">{mp.nombre_plan}</td>
                            <td className="px-6 py-4">
                              {new Date(mp.fecha_matricula).toLocaleDateString("es-ES", {
                                year: "numeric", month: "short", day: "numeric",
                              })}
                            </td>
                            <td className="px-6 py-4">{estadoBadge(mp.estado)}</td>
                            <td className="px-6 py-4">
                              <div className="flex gap-2">
                                {/* Ver detalles */}
                                <motion.button
                                  whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                  onClick={() => { setSelectedMatricula(mp); setModal("details"); }}
                                ...
                                {/* Editar — precarga valores ✅ */}
                                {mp.estado === "Activa" && (
                                  <motion.button
                                    whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                    onClick={() => {
                                      setSelectedMatricula(mp);
                                      setClaseEditada(String(mp.id_clase));
                                      setPlanEditado(String(mp.id_plan));
                                      setEstadoEditado(mp.estado);
                                      setModal("editar");
                                    }}
                                ...
                                <motion.button
                                  whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                  onClick={() => { setSelectedMatricula(mp); setModal("delete"); }}
            ...
            >
              ...
              >
                <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
                  <Eye size={20} /> Detalles de la Matrícula
                </h3>
                <div className="space-y-4 text-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1">{estadoBadge(selectedMatricula.estado)}</div>
                    </div>
                  </div>
                ...
                <div className="flex justify-center pt-4">
                  <button onClick={closeModal} className="px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">Cerrar</button>
                </div>
              </motion.div>
            </motion.div>
          )}

          {/* Modal: Editar — selects precargan valor actual ✅ */}
            ...
            >
              ...
              >
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    >
                      ...
                      {clases.map((clase) => (
                        <option key={clase.id_clase} value={String(clase.id_clase)}>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {planes.map((plan) => (
                        <option key={plan.id_plan} value={String(plan.id_plan)}>
                ...
                <div className="flex justify-center gap-3 pt-4">
                  <button onClick={closeModal} className="px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">Cancelar</button>
                  <button onClick={handleUpdate} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Guardar cambios</button>
                </div>
              </motion.div>
            </motion.div>
          )}
            ...
            >
              ...
              >
                ...
                <div className="flex justify-center gap-3">
                  <button onClick={closeModal} className="px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">Cancelar</button>
                  <button onClick={() => confirmDeleteMatricula(selectedMatricula)} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Eliminar</button>
                </div>
              </motion.div>
            </motion.div>
          )}

          {/* Modal: Confirmar eliminación (paso 2) */}
          {confirmDelete && matriculaToDelete && (
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-4">Esta acción eliminará la matrícula permanentemente. ¿Confirmas?</p>
                <div className="flex justify-center gap-3">
                  <button onClick={cancelDelete} className="px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">Volver</button>
                  <button onClick={() => handleDelete(matriculaToDelete.id_matricula)} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Sí, eliminar</button>
                </div>
              </motion.div>
            </motion.div>
          )}
          ...
          {modal === "nuevaClase" && (
            ...
            >
              <motion.div
                ...
                onClick={(e) => e.stopPropagation()}
              >
                {/* Botón cerrar con ícono X correcto ✅ */}
                ...
                <div className="flex mb-6 bg-gray-100 rounded-lg p-1">
                  <button
                    className={`flex-1 py-2 px-4 rounded-md text-sm font-medium ${modoMatricula === "existente" ? "bg-white text-blue-600 shadow-sm" : "text-gray-600"}`}
                    onClick={() => setModoMatricula("existente")}
                  >
                    Estudiante existente
                  </button>
                  <button
                    className={`flex-1 py-2 px-4 rounded-md text-sm font-medium ${modoMatricula === "nuevo" ? "bg-white text-blue-600 shadow-sm" : "text-gray-600"}`}
                ...
                {modoMatricula === "existente" ? (
                  ...
                  /* Tab: Nuevo estudiante (usuario existente) */
                  <div className="space-y-4">
                    <div>
                    ...
                    <div>
                      ...
                      <input
                        type="number" min="1" max="100" placeholder="Edad"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                        value={edad}
                    ...
                    {/* Sección acudiente (solo menores) */}
