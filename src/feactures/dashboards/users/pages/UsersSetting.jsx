...
export const UsersSetting = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setUserData(data);
        setEditData({ nombre_completo: data.nombre_completo || "", telefono: data.telefono || "" });
  ...
  if (loading) return (
    <Layout><section className="p-[30px]"><div className="text-center text-gray-600">Cargando perfil...</div></section></Layout>
  );

  return (
    <Layout>
      <style>{`
        .msg-wrap { display:inline-flex; align-items:center; gap:.5rem; padding:17px; border-radius:22px; background:#ECFDF5; color:#40c17eff; font-weight:600; will-change:transform,opacity; }
        .msg-entering{ transform:translateX(-120%); opacity:0; animation: slideInFromLeft 400ms cubic-bezier(.2,.9,.2,1) forwards; }
        .msg-visible{ transform:translateX(0); opacity:1; }
        .msg-exiting{ transform:translateX(0); opacity:1; animation: slideOutToLeft 400ms cubic-bezier(.3,.1,.25,1) forwards; }
        ...
        <div className="flex items-start gap-[30px] mt-[80px]">
          ...
          <div className="flex flex-col gap-[10px]">
            <span className="inline-flex justify-center w-[100px] items-center gap-[5px] px-[15px] py-[7px] rounded-full bg-green-100 text-green-700">
              <span className="w-[10px] h-[10px] block bg-[currentColor] rounded-full"></span>Activo
            </span>
            ...
            {isEditingProfile ? (
              <div className="w-[400px] block space-y-4">
                ...
                <label className="mb-[10px] block">
                  <p className="translate-x-[20px] font-bold! italic">Teléfono:</p>
                ...
                <div className="flex gap-[10px] pt-2">
                  <button onClick={handleSaveProfile} disabled={isLoading} className={`btn ${isLoading ? 'bg-blue-100 cursor-not-allowed' : 'bg-blue-100 text-blue-600'} text-blue-500 transition-colors`}>
                    <h4>{isLoading ? 'Guardando...' : 'Guardar cambios'}</h4>
                  </button>
              ...
              <div className="space-y-4 max-w-md">
                ...
                {passwordValidation.currentPassword.message && (
                  <p className={`text-sm mt-1 ${passwordValidation.currentPassword.valid ? 'text-green-600' : 'text-red-600'}`}>{passwordValidation.currentPassword.message}</p>
                )}
                <label className="mb-[10px] block">
                ...
                {passwordValidation.newPassword.message && (
                  <p className={`text-sm mt-1 ${passwordValidation.newPassword.valid ? 'text-green-600' : 'text-red-600'}`}>{passwordValidation.newPassword.message}</p>
                )}
                <label className="mb-[10px] block">
                ...
                {passwordValidation.confirmPassword.message && (
                  <p className={`text-sm mt-1 ${passwordValidation.confirmPassword.valid ? 'text-green-600' : 'text-red-600'}`}>{passwordValidation.confirmPassword.message}</p>
                )}
                <div className="flex gap-[10px] pt-2">
                  <button onClick={handleSavePassword} disabled={isLoading || passwordValidation.newPassword.valid !== true || passwordValidation.confirmPassword.valid !== true || passwordValidation.currentPassword.valid !== true} className={`btn min-w-[220px] flex items-center justify-center ${isLoading || passwordValidation.newPassword.valid !== true || passwordValidation.confirmPassword.valid !== true || passwordValidation.currentPassword.valid !== true ? 'bg-blue-100 cursor-not-allowed text-blue-300' : 'bg-blue-100 text-blue-600'} transition-colors`}>
              ...
              <>
                ...
                <div className="flex gap-4 mt-4">
                  <button onClick={() => setIsEditingProfile(true)} className="btn bg-blue-100 flex items-center gap-[10px] text-blue-600">
                    <h4>Editar perfil</h4>
                  </button>
                  <button onClick={() => setIsChangingPassword(true)} className="btn bg-gray-200 flex items-center gap-[10px] text-gray-700">
